  ...
  .controller('MainCtrl', function ($scope, $http, d3Factory, $filter, $modal, keyFactory) {
    ...
    $scope.getRelation = function(){
      ...
      $http.post('/api/watson', postData).success(function(data) {
        ...
        $scope.index = {};
        // Probably need to build in uid for dataset
        $scope.index.mentions = keyFactory.mentionIndex(data);
        ...
        $scope.mention = function(begin, end){
          var mention = keyFactory.excerpt($scope.relationships.rep.doc[0].text, begin, end)
          return mention;
